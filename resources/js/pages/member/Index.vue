            ...
            <div class="row mb-2 gy-3">
                ...
                <div class="col-sm-6">
                    <div class="float-sm-end gy-3">
                        <button
                            class="btn btn-info btn-sm ms-sm-3 ms-md-3 ms-lg-3 mt-sm-2 mt-3 mt-md-0 mt-lg-0 mt-sm-0"
                            @click="updatePlugin()"
                                        ...
                                        <tr class="custom-table-heading">
                                            <th>
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" @click="checkAll" id="check_all" :checked="(members.length == selected_members.length) && (selected_members.length > 0) ? true : false">
                                                </div>
                                            </th>
                                            <th>Sr No.</th>
                                            <th>Name</th>
                                        ...
                                        <template v-if="members.length > 0">
                                            ...
                                            >
                                                <td style="min-width: 10px">
                                                    <div class="form-check">
                                                        <input class="form-check-input" v-model="selected_members" type="checkbox" :id="`member_${member.id}`" :value="member.id">
                                                    </div>
                                                </td>
                                                ...
                                                <td style="min-width: 100px">
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" type="checkbox" role="switch" :id="`switch_${member.id}`" :checked="member.user_status > 0 ? true : false">
                                                    </div>
                                                </td>
                                                <td style="min-width: 100px">
                                                    {{ member.user_status > 0 ? 'Active' : 'Deactive' }}
                                                </td>
                                                <td style="min-width: 100px">
                                                    {{ member.plugin_version }}
                                                </td>
                                                <td style="min-width: 100px">
                                                    <i :class="member.send_update > 0 ? 'fa fa-check' : 'fa fa-envelope'"></i>
                                                </td>
...
function updatePlugin() {
    confirmAlert({
        ...
        if (result.isConfirmed) {
            axios
                .post(pluginRoutes.bulkPluginUpdate, { 'selected_members' : selected_members.value ?? [] })
                .then((response) => {
                    toastAlert({ title: response.data.message });
                    selected_members.value = [];
                })
...
function deleteMembers() {
    confirmAlert({
        ...
        if (result.isConfirmed) {
            axios
                .post(memberRoutes.deleteMembers, { 'selected_members' : selected_members.value ?? [] })
                .then((response) => {
                    toastAlert({ title: response.data.message });
                    selected_members.value = [];
                    reloadTable();
